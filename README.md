...
보통 게시물에는 "누가 게시물을 작성하였는가?" 라는 정보가 포함되어 있습니다. 그렇기에, 웹 서비스에서는 "한국 사는 철수" 가 올린 게시물을 "미국 사는 스미스" 가 삭제하거나 수정하지 못하도록 해야 합니다.
...
좋아요. 그럼, JWT 란 뭘까요?  
프론트엔드와 백엔드가 분리되어 있는 애플리케이션 개발을 진행하다 보면, JWT 를 통해서 통신할 일이 많습니다.  
...
하지만 이런 토큰을 사용해서 인증을 하는 방법에는 문제가 있습니다. "누군가가 토큰을 가져가 버리면 어떻게 하지?" 이죠. 실제 술집에서는 직원이 신분증의 증명사진과 얼굴을 대조해 보며 이 사람이 실제 신분증을
가지고 있는 사람인지를 확인하지만, 서버는 그렇지 않습니다. 얼굴도, 몸뚱아리도 없는 Request 에서 그를 식별할 수 있는 수단은 헤더에 담겨 있는 토큰 뿐입니다.  
